...
public Transaction canCommit(Transaction trans, Long readTime) {
      Long writeTime = (Long) writeTimes.get(trans.getFileId());
      
      System.out.println(trans.getFileId() + "/" + writeTime);
      
      if(writeTime != null && readTime != null && readTime < writeTime) {
          trans.voteNo();
      } else if(writeTime != null && readTime == null) {
        ...
        trans.voteYes();
        try {
          String transPath = ".\\temp\\" + trans.TransactionId;
          File transFile = new File(transPath);
          OutputStream outStream = new FileOutputStream(transFile);
          ObjectOutputStream transStream = new ObjectOutputStream(outStream);
          transStream.writeObject(trans);
        } catch(Exception e) {
          e.printStackTrace();
        }
      }
    ...
    public void doCommit(Transaction trans) throws IOException, RemoteException{
      if(trans.getOperation() == Transaction.Operation.WRITE) { 
          
          
        put(trans.getFileId(), trans.getStream());
        writeTimes.put(trans.getFileId(), System.currentTimeMillis());
      } else if(trans.getOperation() == Transaction.Operation.DELETE) {
        delete(trans.getFileId());
        writeTimes.remove(trans.getFileId());
      }
    }

    public void doAbort(Transaction trans) throws RemoteException {
      String transPath = "/var/tmp/" + trans.TransactionId;
      File transFile = new File(transPath);
      transFile.delete();
